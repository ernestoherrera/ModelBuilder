...
foreach (Table table in sqlTables)
{
    ...
    SavePoco(tableName, filePath, columns, tabSize);
}

void SavePoco(string className, string fileName, List<Column> columns, int tabSize)
{
    var builder = new StringBuilder();
    var invalidColumnNames = new List<string> { "class", "out", "delegate", "yield", "internal", "volatile", "protected", "private" };
    ...
    using (var streamWriter = new StreamWriter($"{fileName}"))
    {
        ...
        builder.AppendLine("{");
        builder.Append(' ', tabSize).AppendLine($"/// This file is auto-generated. Do not make manual changes to this file");
        builder.Append(' ', tabSize).AppendLine($"/// {className} entity");
        builder.Append(' ', tabSize).AppendLine($"public class {className}");
        builder.Append(' ', tabSize).AppendLine("{");
        ...
        foreach (var col in columns)
        {
            ...
            if (!kvp.Equals(defaultKvP))
                throw new Exception($"Invalid column name: {columnName} for class {className}");

            builder.Append(' ', (tabSize * 2)).AppendLine($"public {propType} {columnName} {{ get; set; }} ");
